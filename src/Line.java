...
public class Line {
    ...
    public boolean isPointOnLine(Point p) {
        ...
        if (this.slope() == null) {
            return ((this.start.getY() < p.getY() && p.getY() < this.end.getY())
                    ...
                    && cmpDouble(this.start.getX(), p.getX());
        }
        if (cmpDouble(this.slope(), 0.0)) {
            return this.isPointInRange(p) && cmpDouble(this.start.getY(), p.getY());
        }
        if (this.isPointInRange(p)
                && cmpDouble(p.getY(), this.slope() * (p.getX() - this.start.getX()) + this.start.getY())) {
            return true;
        }
        return false;
    }
    ...
    public Point intersectionWith(Line other) {
        ...
        if (this.isIntersecting(other)) {
            ...
            if (cmpDouble(intersectionX, Math.round(intersectionX))) {
                intersectionX = Math.round(intersectionX);
            }
            if (cmpDouble(intersectionY, Math.round(intersectionY))) {
    ...
    public Point getEndCloser() {
        ...
        } else if (cmpDouble(this.start.getY(), this.end.getY())) {
